...
//console.log(protoArray2.split(';').sort().join().replace(/,/g, "-"));
//console.log(protoArray3.split(' ').sort().reverse().join().replace(/,/g," "));
...
if (check){
	output=protoArray2.split(';').sort().join().replace(/,/g, "-");
}
...
function reverseSpaces() {
  ...
  //TODO: write the code required for this step
if (check){
	protoArray3.split(' ').sort().reverse().join().replace(/,/g, " ");
}
	return output;
}
